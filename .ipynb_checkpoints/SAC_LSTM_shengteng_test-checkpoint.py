...
parser.add_argument('--save_dir', type=str, default='checkpoints_debug/')
parser.add_argument('--gen_frm_dir', type=str, default='/home/ma-user/work/dataset/317_test_model/')
...
def wrapper_test(model):
    ...
    test_dataloader = x2ms_datasets.DataLoader(test_dataset, batch_size=batch_size, shuffle=False, num_workers=4,drop_last=True)
    ...
    for dat in test_dataloader:
        ...
        ims = preprocess.reshape_patch(ims, args.patch_size)
        # ims.shape (8, 15, 32, 32, 16)
        # real_input_flag (8, 9, 32, 32, 16)
...
# test_dataset_path = r"/cache/test_1024_modif.txt"
...
# model.load()
# model.load_mindspore()


# ms.load_checkpoint(checkpoint_path, model)
model.load_mindspore_custom()

#调试模型
# ims.shape (8, 15, 32, 32, 16)
# real_input_flag (8, 9, 32, 32, 16)
inp = np.random.uniform(-1,1,(8,15,32,32,16)).astype(np.float32)
flag = np.random.uniform(-1,1,(8,9,32,32,16)).astype(np.float32)
