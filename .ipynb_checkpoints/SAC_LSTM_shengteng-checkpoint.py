...
parser.add_argument('--data_train_path', type=str, default="/home/ma-user/work/CIKM_Rardar_Data/train2.txt")
...
parser.add_argument('--pretrained_model', type=str, default='checkpoints/SAC_LSTM_shengteng_32')
...
def wrapper_train(model):
    ...
    for itr in range(1,500000):
        ...
        for ims in train_dataloader:
            ...
            # ims = mindspore.Tensor(ims, dtype=mindspore.float16)
            # real_input_flag = mindspore.Tensor(real_input_flag, dtype=mindspore.float16)
        ...
        if itr % args.test_interval == 0:
            ...
            if valid_mse<best_mse:
                ...
                tolerate = 0
                model.save()
                mox.file.copy_parallel("./checkpoints/SAC_LSTM_shengteng_32", "s3://precipitation-nowcasting/IDA_LSTM-master-huawei_x2ms/checkpoints/SAC_LSTM_shengteng_32/")
            else:
                tolerate = tolerate+1
...
def wrapper_test(model):
    ...
    while flag:
        if index > 1024:
            break
        dat, (index, b_cup) = sample_test(batch_size, data_type='test', index=index)
        dat = nor(dat)
